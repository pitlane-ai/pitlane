...
def test_unique_logger_names_create_separate_instances(tmp_path: Path):
    ...
    logger1 = setup_logger(log1, verbose=False, logger_name="agent_eval_assistant1_task1")
    logger2 = setup_logger(log2, verbose=False, logger_name="agent_eval_assistant2_task1")
...
def test_verbose_mode_adds_stderr_handler(tmp_path: Path):
    ...
    logger1 = setup_logger(log_file, verbose=False, logger_name="agent_eval_test_verbose_off")
    assert len(logger1.handlers) == 1
    assert isinstance(logger1.handlers[0], logging.FileHandler)
    
    # Verbose: file handler + stderr handler
    logger2 = setup_logger(log_file, verbose=True, logger_name="agent_eval_test_verbose_on")
...
def test_collision_detection_prevents_log_mixing(tmp_path: Path):
    ...
    # Setup first logger
    logger1 = setup_logger(log1, verbose=False, logger_name="agent_eval_collision_assistant1_task1")
    logger1.debug("Message from assistant1")
    
    # Setup second logger with DIFFERENT name (correct usage)
    logger2 = setup_logger(log2, verbose=False, logger_name="agent_eval_collision_assistant2_task1")
    logger2.debug("Message from assistant2")
    ...
    with pytest.raises(RuntimeError):
        setup_logger(log3, verbose=False, logger_name="agent_eval_collision_assistant1_task1")
