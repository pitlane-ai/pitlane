...
assistants:
  baseline:
    adapter: claude-code       # or mistral-vibe, opencode
    args:
      model: haiku
  with-skill:
    adapter: claude-code
...
For MCP servers instead of skills, pass the config through adapter args:
...
pitlane run eval.yaml --assistant baseline     # one assistant only
