...
class ClineAdapter(BaseAdapter):
        ...
        try:
            ...
            if result.returncode == 0:
                for line in result.stdout.splitlines():
                    m = re.search(r"Cline CLI [Vv]ersion:\s*(.+)", line)
                    if m:
    ...
    def _build_command(
        ...
        cmd = ["cline", "--act", "--yolo", "--json"]
        if model := config.get("model"):
            cmd.extend(["-m", model])
        if workdir is not None:
            cmd.extend(["--cwd", str(workdir.resolve())])
        cmd.append(prompt)
        return cmd
    ...
    def _parse_output(
        ...
        """Parse JSON events from cline --json.

        Each line is a JSON object with {"type": "say"|"ask", "text": "...", "ts": ...}.
        Collect type=="say" events as assistant messages.
        """
    ...
    def install_mcp(self, workspace: Path, mcp: Any) -> None:
        """Write MCP server config into workspace/.cline/mcp.json.

        Note: Cline reads global MCP config rather than per-workspace config.
        This writes to a workspace-local file as a best-effort approach; it
        may not be picked up automatically without a running Cline instance.
        """
        from pitlane.workspace import _expand_env

        expanded_env = {k: _expand_env(v) for k, v in mcp.env.items()}
        config_dir = workspace / ".cline"
